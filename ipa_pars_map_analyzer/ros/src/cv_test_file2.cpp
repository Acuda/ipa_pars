...
//	int red = (rand() % 250) + 1;
	for (int y = 0; y < map.rows-100; y += 100)
	{
		for (int x = 0; x < map.cols-100; x += 100)
		{
			cv::rectangle(new_map,cv::Point(x,y),cv::Point(x+100,y+100),cv::Scalar(((rand() % 250) + 1),((rand() % 250) + 1), ((rand() % 250) + 1)), -1);
		}
	}
